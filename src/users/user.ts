...
export type circuitNode={ nodeId:number; pubKey:string; }
...
export async function user(userId: number) {
  ...
  _user.get("/getLastCircuit", (req, res) => {
    if (lastCircuit) {const nodeIds = lastCircuit.map(node => node.nodeId);
      res.status(200).json({result: nodeIds});} else {
